...
class TestCSG(unittest.TestCase) :
    ...
    def testQuadraticSurface_intersections_one(self) : 
    ...
    def testUnion_intersections(self) :
    ...
    def testRegion_contains(self) :
        ...
        self.assertTrue(region.node.contains(Point(0.5, 0.5)))
    ...
    def testRegion_intersection_order(self) :
        
        region = self.get_region_2()
        ray = Ray(Point(2, 0.5), Point(-1, 0))
        ints = region.intersections(ray)
        self.assertEqual(len(ints), 2)
        self.assertAlmostEqual(ints[0].x, 1)
        self.assertAlmostEqual(ints[1].x, 0)
        
    #-------------------------------------------------------------------------#
    # TESTS OF GEOMETRY CLASS
    #-------------------------------------------------------------------------#
        
    def get_geometry(self) :
        
        geometry = Geometry(0, 2, 0, 2)
        geometry.add_region(self.get_region())
        geometry.add_region(self.get_region_2())
        return geometry
    
    def testGeometry_find_region_out_of_bounds(self) :
        
        geometry = self.get_geometry()
        self.assertEqual(geometry.find_region(Point(-1,1)), -1)
        self.assertEqual(geometry.find_region(Point(1,-1)), -1)
        self.assertEqual(geometry.find_region(Point(3, 1)), -1)
        self.assertEqual(geometry.find_region(Point(1, 3)), -1)
        
    def testGeometry_find_region_one(self) :
        
        geometry = self.get_geometry()
        ref_inds = geometry.find_region(Point(0.5, 0.5))
        self.assertEqual(len(ref_inds), 1)
        self.assertEqual(ref_inds[0], 1)
        
    def testGeometry_find_region_multiple(self) :
        
        geometry = self.get_geometry()
        ref_inds = geometry.find_region(Point(0.9, 0.9))
        self.assertEqual(len(ref_inds), 2)
        self.assertTrue(0 in ref_inds)
        self.assertTrue(1 in ref_inds)
